...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx;
    uint256 hashBlock;
    // NewYorkCoin: Is this the best value for consensus height?
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the newyorkcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
